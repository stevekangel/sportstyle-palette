...
function colorDist(h1, h2) {
  const p = (h, n) => parseInt(h.slice(n, n+2), 16);
  return Math.sqrt([1,3,5].map(n => (p(h1,n)-p(h2,n))**2).reduce((a,b)=>a+b,0));
}
function scoreColorway(cw, palette) {
  return cw.colors.reduce((s,sc) => s + Math.min(...palette.map(p=>colorDist(sc,p.hex))), 0) / cw.colors.length;
}
...
export default function SportstylePalette() {
  ...
  // Calls our secure API route instead of Anthropic directly
  async function callAPI(body) {
    ...
    if (res.status === 429) throw new Error("Rate limited — wait 30 seconds and try again.");
      ...
      <div style={{maxWidth:1000,margin:"0 auto",padding:"48px 48px 80px"}}>
        <div style={{marginBottom:8}}>
          ...
          <div style={{display:"flex"}}>
            ...
            <input value={handle} onChange={e=>setHandle(e.target.value)} onKeyDown={e=>e.key==="Enter"&&run()} placeholder="emmachamberlain"
              style={{flex:1,background:B.white,border:`1px solid ${B.lineGrey}`,padding:"13px 18px",fontSize:14,color:B.black,outline:"none",fontFamily:"inherit"}}/>
        ...
        <div style={{fontSize:11,color:B.midGrey,letterSpacing:"0.04em",marginBottom:44}}>Enter any public Instagram handle to extract their color palette and match to a GEL-Cumulus 16 colorway</div>

        {error&&<div style={{background:"#FFF3F3",border:"1px solid #F0C0C0",padding:"12px 18px",fontSize:12,color:"#C00",marginBottom:32,fontWeight:500}}>⚠ {error}</div>}
        ...
        {aestheticDesc&&(
          <div style={{marginBottom:44,borderLeft:`3px solid ${B.yellow}`,paddingLeft:20}}>
            <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",color:B.midGrey,marginBottom:8}}>Aesthetic Profile — @{handle.replace(/^@/,"")}</div>
            ...
            <div style={{display:"flex",height:52,marginBottom:14,border:`1px solid ${B.lineGrey}`}}>
              {palette.map((c,i)=>(
                <div key={i} title={`${c.name} — ${c.hex}`} style={{flex:1,background:c.hex,transition:"flex 0.3s ease",cursor:"default"}}
                  onMouseEnter={e=>e.currentTarget.style.flex="2.5"} onMouseLeave={e=>e.currentTarget.style.flex="1"}/>
              ))}
            </div>
            ...
            <div style={{border:`2px solid ${B.black}`,display:"grid",gridTemplateColumns:"260px 1fr",marginBottom:16}}>
              <div style={{background:B.grey,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",borderRight:`2px solid ${B.black}`}}>
                ...
                <div style={{display:"flex",gap:8,marginTop:16}}>
                  {best.colors.map((c,i)=><div key={i} style={{width:24,height:24,borderRadius:"50%",background:c,border:`1px solid ${B.lineGrey}`}}/>)}
                </div>
              </div>
                ...
                {ranked.slice(1).map(cw=>(
                  <a key={cw.sku} href={cw.url} target="_blank" rel="noopener noreferrer"
                    ...
                    <div style={{display:"flex",gap:5}}>
                      {cw.colors.map((c,j)=><div key={j} style={{width:14,height:14,borderRadius:"50%",background:c,border:`1px solid ${B.lineGrey}`}}/>)}
                    </div>
                  </a>
                ))}
              </div>
